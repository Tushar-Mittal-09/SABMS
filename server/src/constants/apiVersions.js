...
const buildVersionPath = (version, path = '') => {
  const cleanVersion = version.startsWith('v') ? version : `v${version}`;
  const cleanPath = path.startsWith('/') ? path : `/${path}`;
  return `${config.apiPrefix}/${cleanVersion}${cleanPath === '/' ? '' : cleanPath}`;
};

const buildCurrentVersionPath = (path = '') =>
  buildVersionPath(CURRENT_VERSION, path);
